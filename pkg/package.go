...
func GetNEVRA(indexEntries []IndexEntry) (*PackageInfo, error) {
	...
	for _, ie := range indexEntries {
		...
		case RPMTAG_BUILDTIME:
			if ie.Info.Type != RPM_INT32_TYPE {
				return nil, errors.New("invalid tag buildtime")
			}
			buildTime, err := parseInt32(ie.Data)
			if err != nil {
				return nil, fmt.Errorf("failed to parse buildtime: %w", err)
			}
		...
		case RPMTAG_PACKAGER:
			if ie.Info.Type != RPM_STRING_TYPE {
				return nil, errors.New("invalid tag package")
			}
		...
		case RPMTAG_GROUP:
			if ie.Info.Type != RPM_I18NSTRING_TYPE && ie.Info.Type != RPM_STRING_TYPE {
				return nil, errors.New("invalid tag group")
			}
		...
		case RPMTAG_CONFLICTNAME:
			if ie.Info.Type != RPM_STRING_ARRAY_TYPE {
				return nil, errors.New("invalid tag conflictname")
			}
			pkgInfo.Conflicts = parseStringArray(ie.Data)
		case RPMTAG_OBSOLETENAME:
			if ie.Info.Type != RPM_STRING_ARRAY_TYPE {
				return nil, errors.New("invalid tag obsoletename")
			}
